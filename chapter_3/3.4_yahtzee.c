...
/* I'm adding in my print function from 3.3 */
void print_array(int array[NUMDICE]);

// These are the functions that we'll be making ^^^
// We'll then work out how to link them to do the exercise
int main(void){
    int dice[NUMDICE];
    int k4 = 0;
    int fh = 0;
...
bool isfullhouse(const int d[NUMDICE]){
    int hist[MAXTHROW] = {0,0,0,0,0,0};
    makehist(d,hist);
    if(histo_has(hist, 3) && histo_has(hist,2)){
	return true;
    }
    return false;
}
